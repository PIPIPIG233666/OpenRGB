/*-----------------------------------------*\
|  LianLiUniHubSLINFController.cpp             |
|                                           |
|  Driver for Lian Li Uni Hub SLINF USB        |
|  lighting controller                      |
|                                           |
|  Oliver P 04/26/2022                      |
...
LianLiUniHubSLINFController::LianLiUniHubSLINFController(hid_device* dev_handle, const char* path, unsigned short pid, std::string dev_name)
{
    dev             = dev_handle;
    dev_pid         = pid;
    location        = path;
...
LianLiUniHubSLINFController::~LianLiUniHubSLINFController()
{
    hid_close(dev);
}

std::string LianLiUniHubSLINFController::GetDeviceLocation()
{
    return("HID: " + location);
}

std::string LianLiUniHubSLINFController::GetFirmwareVersionString()
{
...
std::string LianLiUniHubSLINFController::GetName()
{
    return(name);
}

std::string LianLiUniHubSLINFController::GetSerialString()
{
...
void LianLiUniHubSLINFController::SetChannelLEDs(unsigned char channel, RGBColor * colors, unsigned int num_colors, float brightness)
{
    ...
    for(unsigned int led_idx = 0; led_idx < num_colors; led_idx++)
    {
        ...
        \*---------------------------------------------------------*/
        if(UNIHUB_SLINF_LED_LIMITER && RGBGetRValue(colors[led_idx]) > 153 && (RGBGetRValue(colors[led_idx]) == RGBGetBValue(colors[led_idx])) && (RGBGetRValue(colors[led_idx]) == RGBGetGValue(colors[led_idx])) )
        {
...
void LianLiUniHubSLINFController::SetChannelMode(unsigned char channel, unsigned int mode_value, std::vector<RGBColor> colors, unsigned int num_colors, unsigned int num_fans, bool upd_both_fan_edge, unsigned int brightness, unsigned int speed, unsigned int direction)
{
    ...
    if(num_colors)                                          // Update led_data if there's colors
    {
        switch(mode_value)
        {
            case UNIHUB_SLINF_LED_MODE_STATIC_COLOR:           // Static mode requires a full data array
            case UNIHUB_SLINF_LED_MODE_BREATHING:
                ...
                for(unsigned int i = 0; i < 4; i++)
                {
                    ...
                    | Limiter to protect LEDs                                   |
                    \*---------------------------------------------------------*/
                    if(UNIHUB_SLINF_LED_LIMITER && RGBGetRValue(colors[i]) > 153 && (RGBGetRValue(colors[i]) == RGBGetBValue(colors[i])) && (RGBGetRValue(colors[i]) == RGBGetGValue(colors[i])) )
                    {
...
void LianLiUniHubSLINFController::SendStartAction(unsigned char channel, unsigned int num_fans)
{
...
void LianLiUniHubSLINFController::SendColorData(unsigned char   channel, unsigned int fan_or_edge, unsigned int    num_leds, unsigned char*  led_data)
{
    /*---------------------------------------------------------*\
    | Send edge LED data                                        |
    \*---------------------------------------------------------*/
...
void LianLiUniHubSLINFController::SendCommitAction(unsigned char channel, unsigned int fan_or_edge, unsigned char effect, unsigned char speed, unsigned int direction, unsigned int brightness)
{
    unsigned char usb_buf[65];
